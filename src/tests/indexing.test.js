...
describe('Indexing Business Logic', () => {
  ...
  describe('recordProperty', () => {
    ...
    it('should escape SPARQL string literals', async () => {
      query.mockResolvedValue({})
      await recordProperty('https://example.com/page', 'octo:title', 'a "quoted" \\value')
      const updateCall = query.mock.calls[0][0]
      // Embedded quotes and backslashes must be escaped to keep the literal valid.
  ...
  describe('handleMention', () => {
    it('should create mention and backlink when new', async () => {
      ...
      const triples = insert.mock.calls[0][0]
    ...
    it('should run existence checks in parallel and batch writes into a single insert', async () => {
      ...
      expect(queryBoolean).toHaveBeenCalledTimes(5)
  ...
  describe('handleHTML', () => {
    ...
    it('should use uri as subject when harmonizer returns "source"', async () => {
      ...
      const titleUpdate = query.mock.calls.find(call => call[0].includes('octo:title'))
    ...
    it('should record postDate from harmonized output', async () => {
      ...
      const updateCalls = query.mock.calls.map(c => c[0])
