...
1. **Scope. — RESOLVED (2026-07-08): broad scope as a *lazy policy*.** The choice is not "relationships-only vs wrap every triple." The resolution: **RDF-star is OP's single uniform mechanism for per-statement metadata, applied only to statements that actually carry metadata.** A triple gets a quoted-triple annotation *iff* it has per-statement metadata; plain facts stay plain. So the real boundary is **statements-with-metadata vs statements-without**, not relationships-vs-everything. What has metadata: typed relationships (`type`, `terms`, `created`), and **content labels #192** (value + source + timestamp + negation — structurally identical to a relationship-with-metadata, which is why narrow scope would force a *second, parallel* metadata mechanism for them). What doesn't: `documentRecord` leaves (mimetype/size — just facts; provenance like `addedBy` is a sibling leaf, not an annotation), SKOS term facts (`prefLabel`, `inScheme`). **Consequences:** (a) the "assert both" invariant (item 2) becomes a single global rule; (b) `documentRecord` projection (#237) is unaffected — it reads asserted base triples with plain patterns, and a SPARQL-star annotation is only matched by explicit `<< … >>` syntax, so it can never be picked up accidentally; (c) **#192 labels then ride on the RDF-star migration** rather than shipping independently — name this coupling. The lazy application means the elegance costs nothing on plain-fact storage or leaf-read speed.
...
**Relationship-terms are the RDF-star × SKOS composition point.** "Terms" appear in two structurally different shapes — keep them distinct:
- **Term as the *object* of a tag edge** (e.g. `Record octothorpes <relay/~/landscape>`): the Term is what the edge points at; the base triple stays plain and traversable. This is what a tagging client (e.g. Memex) uses. Only the edge's *type* metadata relocates under RDF-star.
- **Relationship-terms proper** — terms as *metadata qualifying another edge* (e.g. "A cites B **about gadgets**"): the term annotates the citation. Under RDF-star this is `<< A octothorpes B >> octo:hasTerm <relay/~/gadgets>`.

Relationship-terms are relationship *metadata*, so they migrate under RDF-star in **both** scopes (they're in the relationship bundle regardless of the item-1 labels decision). Their payoff is the composition: the annotation *value* is a shared **SKOS Concept node**, so `<< A octothorpes B >> octo:hasTerm <term>` lets you ask "all citation edges tagged `gadgets`" and join on the term hub — statement-about-a-statement whose metadata is a shared concept. This is the substrate for the deferred **"discover targets by relationship terms"** query mode (`backlinked+thorped?o=gadgets` with no `?s`).

**Query blast radius:** the existing relationship-term query paths — the `+thorped` / `relationTerms` filters in `getStatements`/`queryBuilders` that read terms off the blank node — get rewritten to quoted-triple + annotation patterns, sharing fate with the subtype-path queries (#236). Another reason the Memex core epic (#240) precedes the RDF-star rewrite.
